...
//     copy query to vector
//    vector<WordID> q(query.begin(), query.end());
//# pragma omp parallel for reduction(+:score) num_threads(6)
    for ( set<WordID>::iterator it = query.begin(); it != query.end(); ++it ){
//    for (unsigned j = 0; j<q.size(); j++ ) {
//      double this_score = docIter->second.getScoreForQueryTerm( q[j] );
